#!/usr/bin/env python3
"""
用 QA Test Skills 根 SKILL.md 完整执行 12 步工作流，生成电商案例测试用例。
用法：
  DS_KEY=sk-xxx python scripts/gen_example_cases.py             # DeepSeek
  NVIDIA_KEY=nvapi-xxx python scripts/gen_example_cases.py      # NVIDIA
"""
...
EXAMPLE = ROOT / 'examples' / 'ecommerce-project'

# Provider 配置（优先 NVIDIA，回退 DeepSeek）
...
def build_user_message():
    ...
    parts = []
    parts.append(f"请帮我完整测试这个电商项目：{EXAMPLE / 'docs' / 'prd.md'}")
    parts.append("\n===== 主需求文档 prd.md =====")
    parts.append((EXAMPLE / 'docs' / 'prd.md').read_text(encoding='utf-8'))
    req_dir = EXAMPLE / 'docs' / 'requirements'
    for f in sorted(req_dir.glob('*.md')):
        parts.append(f"\n===== {f.name} =====")
        parts.append(f.read_text(encoding='utf-8'))
    # 不加任何执行要求/模块数量/格式约束——按 SKILL.md 的 12 步工作流自行决策
    return '\n'.join(parts)


def main():
    if not API_KEY:
        print('❌ 请设置环境变量 DS_KEY')
        sys.exit(1)

    system_prompt = SKILL_FILE.read_text(encoding='utf-8')
    user_message = build_user_message()
    print(f'📄 System prompt: {SKILL_FILE.name} ({len(system_prompt)} chars)')
    print(f'📄 User message: {len(user_message)} chars（含 6 模块需求）')
    print('🤖 调用 DeepSeek 执行 12 步工作流...\n')

    output, usage = call_llm(system_prompt, user_message)
    print(f'✅ 生成完成：{len(output)} chars, {usage.get("total_tokens", 0)} tokens')

    out_file = EXAMPLE / 'docs' / 'workflow-output.md'
