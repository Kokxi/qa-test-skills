...
description: >-
  缺陷生命周期管理，管理缺陷从发现到关闭的全流程，包含状态流转、分级和度量指标。当用户需要管理缺陷流程、Bug管理或缺陷处理时自动触发。
  也适用于：团队需要规范缺陷管理流程、或分析缺陷趋势改进质量时。
   关键词：缺陷管理、Bug生命周期、状态流转、缺陷度量、严重度分级、缺陷分类、缺陷趋势、闭环管理、缺陷归因。
...
**核心原则**：缺陷管理不仅仅是记录Bug，而是驱动问题解决和质量改进。
本技能覆盖缺陷生命周期、分级标准、分析方法和度量指标。
