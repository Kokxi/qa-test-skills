...
description: >-
   回归测试策略与用例筛选，根据变更范围、风险等级和时间约束制定精准回归方案。当用户需要做回归测试、选择回归用例或制定回归策略时自动触发。
   也适用于：版本迭代需要确定回归范围、回归时间不够需要优先级排序、或回归测试效率需要优化时。
   关键词：回归测试、回归策略、用例筛选、回归范围、冒烟测试、全量回归、增量回归、回归优先级、变更覆盖、回归效率、回归金字塔。
...
input_format: 变更分析结果 + 风险评估 + 用例库
output_format: 回归测试方案（分级策略 + 用例清单 + 执行计划）
---
