...
description: >-
  启发式检查清单，提供不同类型功能的测试要点模板。当用户面对未知类型功能不知道如何入手、需要知道有什么测试要点或需要checklist时自动触发。
  也适用于：测试新人需要功能测试指引，或快速评估功能测试覆盖完整性时。
   关键词：启发式清单、测试要点、功能测试、Checklist、测试模板、功能类型、表单测试、购物车测试、支付测试、导入导出测试、审批测试、权限测试、测试覆盖矩阵。
