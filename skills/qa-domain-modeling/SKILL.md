...
description: >-
  领域建模，构建状态机、数据流、服务依赖图以理清复杂业务流程。当用户需要画状态图、理清数据流、梳理服务依赖或理解复杂业务流程时自动触发。
  也适用于：需求文档复杂难以理解，或需要可视化呈现测试范围时。
   关键词：领域建模、状态机、数据流、服务依赖、状态转换图、数据流图、服务依赖图、业务建模、系统建模、模型驱动测试。
...
**核心原则**：不只是画流程图，而是画出状态机、数据流向、一致性约束点。
本技能提供三种建模视图（状态转换图/数据流图/服务依赖图）和输出格式。
