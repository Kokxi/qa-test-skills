...
> **49个技能（入口工作流 qa-test-skills + 48个专家级子技能，共 49 个 SKILL.md 文件），覆盖测试全生命周期** | 让初级测试人员输出专家级测试用例 | AI辅助测试设计最佳实践
...
48个技能不是随意堆砌的。`skills/qa-test-skills/SKILL.md`（入口工作流）将它们编排为12步有序流水线（含输入验证 → 输出验证 → 专家评审），每一步的输入输出都有明确依赖关系。新人跟着工作流走一遍，就理解了一个完整的测试设计过程。
...
├── docs/
│   ├── prd.md                    # 主需求文档（包含索引引用）
│   └── requirements/             # 6 个子模块需求
├── test-cases.md                 # 120 条规范化测试用例
...
- **完整测试用例**：120条测试用例，覆盖认证/用户/商品/订单/支付/库存/非功能 7 大模块
...
> 📋 **完整技能清单**：以上为核心/专项/管理代表技能。完整 49 个技能的
> 四维分类（可单独使用 / 依赖上游 / 有文件输出 / 经验型无输出）及依赖关系
> 见 [`docs/skill-classification.md`](docs/skill-classification.md)。
...
```
┌─────────────────────────────────────────────────────────────┐
│  第0步：需求文档解析 qa-input-validation（含输入验证，防幻觉）│
...
> 上面是 AI 内部执行的 12 步；以下是用户在实际工作中如何配合使用：
...
| **核心流程** | 15个 | 需求评审→需求解构→场景构建→深度设计→回归策略设计→...→测试报告→专家评审 |
| **专项测试** | 4个  | 接口测试、Agent测试、移动端测试、专项测试                           |
| **支撑流程** | 7个  | 测试策略、自动化架构、CI/CD、发布风险、测试左移、测试右移、测试估算 |
