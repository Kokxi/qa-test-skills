...
name: qa-test-skills
description: >-
  测试工作流编排引擎 + 48个专家级子技能集。自动串联qa-input-validation到qa-test-reporting的全部技能，生成专家级测试用例。
  当用户要求生成测试用例、请求测试帮助、上传需求文档或提供URL时自动触发。
  也适用于：用户不确定如何开始测试、需要完整测试流程而非单个技能时、想浏览技能集目录、了解QA Test Skills包含哪些技能、查看技能分类说明。
  工作流按序执行：输入验证→需求评审→需求解构→场景构建→深度设计→上下文工程→提示词生成→AI生成→输出评审→盲区补盲→测试报告。
  关键词：生成测试用例、帮我测试、测试设计、需求文档测试、完整测试流程、工作流编排、AI测试生成、自动串联、测试全流程、技能目录、技能清单、安装指引。
when_to_use: >-
...
5. 调用 qa-regression-testing（回归策略设计）
   输出文件：回归策略.md
...
11. 调用 qa-output-validation（输出验证）→ 如果验证失败，返回问题清单
    输出文件：输出验证报告.md

12. 调用 qa-test-reporting（测试报告）
    输出文件：测试报告.md、测试用例.csv

13. [可选] 调用 qa-expert-review（专家评审）
    输出文件：专家评审报告.md
...
- 每个步骤完成后必须检查输出文件是否存在
- 如果输出文件不存在，必须重新执行该步骤
- 不得跳过任何步骤，特别是步骤7（提示词生成）和步骤9-10（输出评审与补盲）
